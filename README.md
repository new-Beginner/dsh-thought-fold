...
DeepSeek Harness 的思考与工具过程折叠增强插件。**2.2.0 增加中途插话分段和中断过程折叠**，保留 2.1.0 的底部/浮动收起和历史会话兼容能力，仍不向模型注入任何指令。
...
- **插话处分段**：已进入聊天记录的 `user` / `steering` 消息始终保留在分段外，不让过程组跨过你的补充要求。连续插话不生成空组。
- **前段确认结束才收起**：任务仍运行时，只有前段各步骤已结束、工具已结算，且结束位置不晚于插话边界，才默认收起前段；尾段和未完成前段保持可见。排队中、尚未进入聊天流的插话不触发分段。
- **中断无需最终回答锚点**：宿主确认轮次已结束且有可靠结束边界时，可以折叠中断过程。保留最后一段有非空正文的已输出回答，以及用户消息、停止/错误/长度限制提示。
- **不把断网等待当中断**：等待重试、审批或尚无结束确认时，不因长时间没输出就折叠正在执行的过程。
- **独立思考不重复包组**：仅包含一个纯思考消息的候选段继续使用原生思考控件；整段过程仍有底部/浮动入口。
- **一套控件接管**：需要分段时暂时隐藏原生整轮入口，用可撤销的显示标记让分段独立控制过程行；不通过点击强制改变原生展开状态。退出分段或停用插件时撤销显示标记。
- **段级状态稳定**：用右侧明确边界标识分段；手动展开不会被普通重绘或历史前插重置。浮动收起注明轮次与段号，始终只操作当前段。
...
6. **不猜测边界**：缺少可靠轮次/步骤结束元数据、尚未结束的当前段、未知旧版渲染结构保持可见；不按“最后一段文字看起来像回答”推断。中断时依据宿主的正文 block 类型保留已输出文本，不推断其是否完整。
...
可通过 DSH 的插件管理界面安装本地打包文件 `dsh-thought-fold-2.2.0.tgz`。保留已有的 2.1.0 / 2.0.0 安装包可回退。
...
- `src/fold-runtime.js`：会话作用域控制器、原生/兼容组、可撤销显示接管及浮动定位。
- `src/process-plan.js`：仅接收标量事实的分段与中断边界判定。
...
`npm test` 包含 Host 回归、设置生命周期、分段边界用例、客户端 VM 检查以及生成产物一致性检查。`npm pack` 前会自动构建。
