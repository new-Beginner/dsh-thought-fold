window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function syncRuntimeAppearance(config) {
      ...
      return h('div', { className: 'dsh-tf-settings' },
        ...
        h('section', { className: 'dsh-tf-card', 'aria-labelledby': 'dsh-tf-runtime-title' },
          ...
          h('div', { className: 'dsh-tf-assurance' },
            ...
            h('span', null, '任务完成后的过程折叠由 DSH 通用设置中的“紧凑会话视图”控制。')
          )
        ),
        h('section', { className: 'dsh-tf-card', 'aria-labelledby': 'dsh-tf-appearance-title' },
          ...
          h(SettingRow, { label: '增强运行状态样式', description: '使用静态 CSS 优化工具调用与过程折叠控件，不扫描或重写聊天内容。' },
        ...
        h('div', { className: 'dsh-tf-assurance' },
          ...
          h('span', null, '无指令注入 · 无 DOM 扫描 · 使用 DSH 原生过程折叠')
        )
      );
    }
    ...
    return {
      ...
      apply(ctx) {
        ...
        requestSettings()
          .then((payload) => {
            if (active && runtimeOwner === owner) {
              syncRuntimeAppearance({ ...DEFAULT_CONFIG, ...(payload.config || {}) });
            }
          })
          .catch(() => {
            if (active && runtimeOwner === owner && typeof document !== 'undefined' && document.documentElement) {
              document.documentElement.removeAttribute(ROOT_ATTRIBUTE);
            }
          });
